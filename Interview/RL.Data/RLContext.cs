    ...
    protected override async void OnModelCreating(ModelBuilder builder)
    {
        ...
        builder.Entity<PlanProcedure>(typeBuilder =>
        {
            ...
            typeBuilder.HasOne(pp => pp.Procedure)
                .WithMany(p => p.PlanProcedures) // <-- This was missing
                .HasForeignKey(pp => pp.ProcedureId);
        });

        builder.Entity<AssignedUser>(typeBuilder =>
        {
            typeBuilder.HasKey(assigned => new { assigned.UserId, assigned.ProcedureId, assigned.PlanId });
            typeBuilder.HasOne(assigned => assigned.User).WithMany(p => p.AssignedUsers);
            typeBuilder.HasOne(assigned => assigned.Plan).WithMany(p => p.AssignedUsers);
            typeBuilder.HasOne(assigned => assigned.Procedure).WithMany(p => p.AssignedUsers);
        });
